...
function renderWithStatementContext(ui: ReactElement) {
  ...
  render(
    <StatementContext.Provider value={{ ...mockContextValue }}>{ui}</StatementContext.Provider>
  );
}
...
describe("StatementFilter", () => {
  ...
  it("should display a message for no matches", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("Category:"), "Transfer");

    await user.type(screen.getByPlaceholderText("Search"), "Sputify");
  ...
  it("should close the modal when the filtering has matching transactions", async () => {
    const user = userEvent.setup();
    renderWithStatementContext(<StatementFilter />);
  ...
  it("should display error message for invalid search input", async () => {
    ...
    await user.type(screen.getByPlaceholderText("Search"), "21232");
