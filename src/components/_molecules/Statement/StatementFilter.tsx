...
export function StatementFilter() {
  ...
  function onSubmit({ category, date, term, type }: FormFilter) {
    ...
    const byCatergory =
      category === "All"
        ? byDate
        : byDate.filter((transaction: Transaction) => transaction.type == category);

    const byType =
      type === "All"
        ? byCatergory
        : byCatergory.filter((transaction: Transaction) => {
            if (type === "Debit") {
    ...
    const byTerm = byType.filter((transaction: Transaction) => {
      return transaction.description.includes(term);
    });

    if (byTerm.length === 0) {
      setHasNoMatch("No matches.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex justify-end gap-3 items-center">
      <ButtonComponent
